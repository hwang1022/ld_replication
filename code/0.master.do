...
****
	...
	if `install_packages' == 1 {
		local packagelist baltab strclean
...
****
	...
	*******************
	if `run_cleaning' == 1 {
		...
		* Note that there is a Python script that needs to be run first to download weather data. See `6_weather/1_open_meteo_api_history.py`. Currently the there is no need to run this script as the data is already downloaded.

		// !python "$code/6_weather/1_open_meteo_api_history.py"
