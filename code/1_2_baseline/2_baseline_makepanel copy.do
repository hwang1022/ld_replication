...
* Last modified: 2026-04-17 (HW)
**************************************************

* This dofile tests whether changing the recall max depth to default 7 and handling of 999 and var by var vs entire row imputation affects the results
...
****	
	...
	* Change 999s to missing 
	forvalues z = 1/7 {
		foreach var in `numericVars' {
			replace bs_`var'_`z' = . if bs_`var'_`z' == 999
		}
...
*******************	  
	...
	sort pid date 
	save "$temp/04_baseline_makepanel_hw_test.dta", replace



cf _all using "$temp/04_baseline_makepanel.dta"
