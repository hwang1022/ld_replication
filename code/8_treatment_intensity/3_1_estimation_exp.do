...
* 	Author:				HW 
* 	Created:			2025-May-29 (HW)
* 	Last modified:		2025-May-30 (HW)
************************************************************
************************************************************

	// Old dataset
	// local baseline_panel $datadir/03. Baseline/02. Output/04_baseline_makepanel.dta
			...
			forval j = 0/1 {
				...
				preserve
					...
					* Set up survival data and run streg
					stset time_at_stand_t, failure(event)
				...
				* Sum survivors from all previous arrival cohorts
				forval j = 0/`=`i'-1' {
					...
					* where S_f,g(t) = exp(-λ_f,g × t)
					...
					local survival_found = exp(-`lambda_found' * `time_diff')
					local survival_not = exp(-`lambda_not' * `time_diff')
...
******************
	...
	* Rename
	rename raw_stand_size 			raw_stand_size_exp
	rename corrected_stand_size 	corrected_stand_size_exp

	* Save results
	keep stand correction_factor raw_stand_size_exp corrected_stand_size_exp
	save "$temp/estimated_stand_sizes_exp.dta", replace
