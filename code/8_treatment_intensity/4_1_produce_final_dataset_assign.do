...
***************************************
	...
	use pid stand treatment using "$raw/ls_randomization_master.dta" , clear
	drop if inlist(stand, ${droplist})
	keep if treatment == 1
	...
	save "$temp/num_treat_by_stand.dta" , replace
...
******************
	...
	lab var num_rid "Workers Approached"
	lab var num_treatment "Assigned Treatment"
	lab var final_stand_size_exp "Estimated Stand Size"
	lab var treat_intensity_exp "Treatment Intensity"


	save "$final/stand_size_intensity.dta" , replace 
