...
// 	replace firstofshock1_calwk_j = firstofshock1_calwk_j+2 if firstofshock1_calwk_j == 24
	...
	* Three+ weeks post shock
	...
	reg attend_nadj treat treatXpost_attendloo_b25 post_attendloo_b25 treat##firstwk_attendloo_b25 attend_week bl_attend bl_earn  bl_modalwage week_in_dm i.standid i.strata i.calendar_week if phase==2 , vce(cluster standid)
	reg attend_nadj treat treatXpost_attendloo_b25 post_attendloo_b25 treat##firstwk_attendloo_b25  if phase==2 , vce(cluster standid)
...
gen temp = week_in if firstwk_attend_j==1 & phase==2
egen first_shock_weekin = min(temp), by(pid)

/*----------------------------------------------
   2. Construct cohort variables
----------------------------------------------*/
...
/* Binary indicator for never-treated units (used in control_cohort()) */
gen never_treat = (cohort == 0)

tabstat never_treat if unique_obs == 1, by(standid) stats(mean N)
