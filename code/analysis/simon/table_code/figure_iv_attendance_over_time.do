...
binscatter attend_adj_resid2 bins1, by(treatment) discrete colors(navy maroon) msymbols(O X) ///
	...
	ytitle("Weekly Mean Attend", size(small)) xtitle("Weeks in Phase", size(small)) yscale(range(-1 1.5)) xlab(0(5)40) ///
...
graph export "$data/output/figures/attend_adj_bs_p1_p2_p3_stand_calweek_v2_noci.pdf", replace
graph export "$data/output/figures/figure_iv_noci.pdf", replace
...
twoway (connected mean bins1 if treatment == 1, lc(maroon) mcolor(maroon)) (rcap lb ub bins1 if treatment == 1, lc(maroon)) ///
	...
	text(1.5 4 "{bf:Phase 1}", size(small)) text(1.5 11.5 "{bf:Phase 2}", size(small)) ///
	text(1.5 20 "{bf:Phase 3}", size(small)) text(1.5 0 "{bf:BL}", size(small))
graph export "$data/output/figures/attend_adj_bs_p1_p2_p3_stand_calweek_v2.pdf", replace
graph export "$data/output/figures/figure_iv.pdf", replace
