...
gen bins2 = week_in
replace bins2 = week_in + 7 if phase == 2
replace bins2 = week_in + 15 if phase == 3
...
preserve
local max_week 23
keep if bins2 != . & bins2 < `max_week'

	* Build x-axis labels: map cumulative bins2 positions to week-within-phase
	local xlabs
	forvalues i = 0/`max_week' {
		if `i' <= 7 {
			local lbl = `i'
			local xlabs `xlabs' `i' "`lbl'"
		}
		else if `i' <= 15 {
			local lbl = `i' - 7
			local xlabs `xlabs' `i' "`lbl'"
			...
			local xlabs `xlabs' `i' "`i'"
		}
		
	}
binscatter attend_adj_resid2 bins2, by(treatment) discrete colors(navy maroon) msymbols(O X) ///
	...
	ytitle("Weekly Mean Attend", size(small)) xtitle("Weeks in Phase", size(small)) yscale(range(-1 1.2)) ylab(, nolabels) xlab(`xlabs') ///
...
binscatter attend_adj_resid2 bins1, by(treatment) discrete colors(navy maroon) msymbols(O X) ///
	...
	text(1.2 20 "{bf:Phase 3}", size(small)) text(1.2 0 "{bf:BL}", size(small))
graph export "$output/figures/attend_adj_bs_p1_p2_p3_stand_calweek_v2_noci.pdf", replace
graph export "$output/figures/figure_d_old.pdf", replace
...
graph export "$output/figures/figure_d_ci.pdf", replace
